...
void IStarUI::StaticLoadResource()
{
...
void IStarUI::Initialize(const std::string& texturehandle)
{
	for (int i = 0; i < 2; i++)
	{
...
void IStarUI::UpdateNumber(int32_t status)
{
...
void IStarUI::AppLock()
{
	mAppMode = true;
	alphaTimer.mElapsedTime = alphaTimer.mMaxTime;
}

void IStarUI::Appearance(float time)
{
	if (!mAppMode)
	{
		alphaTimer.Reset();
...
void IStarUI::DisAppearance(float time)
{
	if (mAppMode)
	{
		alphaTimer.Reset();
